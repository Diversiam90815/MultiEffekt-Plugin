...
class BuildRunner(object):
    ...
    def __init__(self):   
        parser = argparse.ArgumentParser(description='This tool can be used to build and deploy the project.')
    ...
    def _get_current_project_version(self):
        cmake_file_path = os.path.join(self.args.path_project, "plugin", "CMakeLists.txt")
        pattern = r'set\(PLUGIN_VERSION\s+(\d+\.\d+\.\d+)\)'
    ...
    def _update_app_version_in_cmake(self, project_version):
        pattern = r'set\(PLUGIN_VERSION\s+(\d+\.\d+\.\d+)\)'
        cmakeFile = os.path.join(self.args.path_project, "plugin", "CMakeLists.txt")
        ...
        with open (cmakeFile, 'r') as fileIn, open (tmpFile, 'w') as fileOut:
            for line in fileIn:
                match = re.search(pattern, line)
                if match:
                    fileOut.write(f'set(PLUGIN_VERSION {project_version})\n')
                else:
                    fileOut.write(line)
